"""Export false-positive paths on per-source negative target sets (1-hop, 2-hop, 3-hop)."""
...
def load_deg_universe_and_pos_for_source(deg_dir: str, raw_source_gene: str, p_threshold: float, prefer_fdr: bool):
    """Return (all_targets, pos_targets, stats_map, error_msg)."""
    ...
    for tgt_label in neg_targets:
        ...
        for s in iter_incdec_statements_on_edge(G, src0, tgt0):
            ...
            rows.append({
                ...
                "stmt_type": s.get("stmt_type"), "belief": s.get("belief"),
...
def run_2hop_for_source(G, src_label: str, neg_targets_set: set[str],
                        ...
                        stats_map: dict, pv: PermutationView | None):
    rows = []
    src0 = (src_label if pv is None else pv.orig_for_label(src_label))
    if not src0 or src0 not in G:
        return rows
    ...
    for mid0 in G.successors(src0):
        ...
        for tgt0 in G.successors(mid0):
            ...
            st = stats_map.get(tgt_label, {})
            rows.append({
                "source": src_label, "intermediate": mid_label, "target": tgt_label,
    ...
    for m10 in G.successors(src0):
        ...
        for m20 in G.successors(m10):
            ...
            for tgt0 in G.successors(m20):
                ...
                st = stats_map.get(tgt_label, {})
                rows.append({
                    "source": src_label, "intermediate_1": m1_label,
                    "intermediate_2": m2_label, "target": tgt_label,
                    "stmt_type_1": hop1_stmt.get("stmt_type"),
                    "stmt_type_2": hop2_stmt.get("stmt_type"),
...
def main():
    ap = argparse.ArgumentParser(
        description="Export FP paths using per-source DEG universes (1-hop + 2-hop + 3-hop).",
    )
    ...
    if args.mode == "1hop":
        require(args.output_1hop, "--output-1hop")
    elif args.mode == "2hop":
        require(args.output_2hop, "--output-2hop")
    elif args.mode == "3hop":
        require(args.output_3hop, "--output-3hop")
    elif args.mode == "both":
        require(args.output_1hop, "--output-1hop")
        require(args.output_2hop, "--output-2hop")
    elif args.mode == "all":
    ...
    logger.info("HGNC nodes in graph: %d", len(hgnc_nodes))

    allowed_intermediates = None
    if args.allowed_intermediates_csv:
        allowed_intermediates = load_gene_set_from_csv(
            args.allowed_intermediates_csv, gene_column=args.allowed_intermediates_gene_col,
        )
        allowed_intermediates = {g for g in allowed_intermediates if g in G and is_hgnc_node(G, g)}
        logger.info("Allowed intermediates loaded (in-graph HGNC): %d", len(allowed_intermediates))

    pv = None
    if args.permute_seed and args.permute_seed > 0:
        pv = PermutationView(G=G, hgnc_nodes=hgnc_nodes, seed=args.permute_seed)
        logger.info("Evaluating on PERMUTED network labels (seed=%d)", args.permute_seed)

    sources_raw = load_sources_from_target_validation(
        source_genes_csv=args.source_genes_csv, filter_column=args.filter_column,
        filter_value=args.filter_value, gene_column=args.gene_column,
    )
    logger.info("Sources in genes CSV: %d", len(sources_raw))
    ...
    for raw_src in sources_raw:
        ...
        if args.limit_negatives_per_source and args.limit_negatives_per_source > 0 and len(neg) > args.limit_negatives_per_source:
    ...
    logger.info("Jobs prepared: %d sources | skipped: %d", len(jobs), skipped)
    logger.info(
        "Mode: %s | Negatives per source: %s",
        args.mode, "ALL" if args.limit_negatives_per_source == 0 else str(args.limit_negatives_per_source),
    )
    ...
    logger.info("Running false-positive pathfinding...")
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futs = [ex.submit(job_fn, j) for j in jobs]
        for i, fut in enumerate(as_completed(futs), start=1):
            out1, out2, out3 = fut.result()
            if out1:
                onehop_rows.extend(out1)
            if out2:
                twohop_rows.extend(out2)
            if out3:
                threehop_rows.extend(out3)
            if i % 25 == 0 or i == len(futs):
                logger.info(
                    "  progress %d/%d | 1hop_rows=%d | 2hop_rows=%d | 3hop_rows=%d",
                    i, len(futs), len(onehop_rows), len(twohop_rows), len(threehop_rows),
                )
    ...
    if args.mode in ("1hop", "both", "all"):
        ...
        logger.info("Wrote 1-hop -> %s | rows=%d", args.output_1hop, len(df1))
    ...
    if args.mode in ("2hop", "both", "all"):
        ...
        logger.info("Wrote 2-hop -> %s | rows=%d", args.output_2hop, len(df2))

    if args.mode in ("3hop", "all"):
        df3 = pd.DataFrame(threehop_rows)
        if not df3.empty:
            df3 = df3[df3["source"] != df3["target"]].copy()
        df3.to_csv(args.output_3hop, index=False)
        logger.info("Wrote 3-hop -> %s | rows=%d", args.output_3hop, len(df3))
