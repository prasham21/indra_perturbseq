"""Legacy script: cell_count_visualization."""
...
def create_full_report(gene_list):
    ...
    for g in gene_list:
        rows.append({
            ...
            "analysis_flag": flag_map.get(g, "NOT_PRESENT")
        })
...
def main():
    ...
    ap.add_argument("--adata-all-with-guides-raw-h5ad", default="adata_all_with_guides_raw.h5ad", help="Path: adata_all_with_guides_raw.h5ad")
    ...
    adata_path = "adata_all_with_guides_raw.h5ad"
    adata = sc.read_h5ad(adata_path)
    cell_counts = adata.obs["Gene"].value_counts().to_dict()

    # Load Validation CSV
    val_path = "target_validation_expanded.csv"
    df = pd.read_csv(val_path)

    val_genes = df["Gene"].astype(str).tolist()
    flag_map = df.set_index("Gene")["analysis_flag"].to_dict()
    ...
    logger.info("\n==================== FULL GWAS REPORT ====================")
    logger.info(gwas_report.to_string(index=False))

    logger.info("\n==================== FULL NON-GWAS REPORT ====================")
    logger.info(non_gwas_report.to_string(index=False))
