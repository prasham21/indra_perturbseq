"""Fold-change vs p-value landscape plots faceted by hop distance."""
...
def main():
    ...
    ap.add_argument("--deg-dir", required=True, help="de_results_per_gene directory")
    ...
    tv = pd.read_csv(args.tv, usecols=["Gene", "analysis_flag"])
    sources = set(
        tv.loc[tv["analysis_flag"] == "Use_for_analysis", "Gene"].astype(str).str.strip()
    )
    sources -= {"TP53", "CDKN1A"}

    rows = []
    for s in sorted(sources):
        f = os.path.join(args.deg_dir, f"{s}_vs_control.csv")
    ...
    for grp in groups_to_trim:
        ...
        after = len(sub)
        logger.info(
            "%s: removed %d / %d rows (top %s%% pval outliers)",
            grp, before - after, before, trim_pct,
        )
    ...
    for i, hop in enumerate(order):
        ...
        ax.set_title(
            f"Highlighting {hop} (top {trim_pct}% outliers removed from 2-hop/3-hop/3+)"
        )
    ...
    plt.suptitle(
        f"FC vs P-value landscape (top {trim_pct}% outliers removed from 2/3-hop/3+)",
        fontsize=13,
        y=1.01,
    )
    plt.tight_layout()
