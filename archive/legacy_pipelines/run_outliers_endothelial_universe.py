"""Run outliers (TP53/CDKN1A) for 1-hop + 2-hop on endothelial target universe."""
...
def load_endothelial_gene_set(path: str, gene_column: str = "gene") -> set[str]:
    df = pd.read_csv(path, low_memory=False)
    if gene_column not in df.columns:
        raise ValueError(f"Endothelial list must have column '{gene_column}'. Columns: {df.columns.tolist()}")
...
def load_targets_from_deg_as_universe(deg_csv: str) -> tuple[set[str], dict]:
    """Return (universe_targets, stats_map) from DEG file."""
...
def main():
    ap = argparse.ArgumentParser(
        description="Run outliers only (TP53/CDKN1A) for 1-hop + 2-hop on endothelial target universe.",
    )
    ...
    ap.add_argument("--deg-csvs", nargs="+", required=True, help="Corresponding <source>_vs_control.csv files")
    ...
    logger.info(
        ...
        secs / 60, G.number_of_nodes(), G.number_of_edges(),
    )

    endothelial = load_endothelial_gene_set(args.endothelial_list, gene_column=args.endothelial_gene_col)
    endothelial = {g for g in endothelial if g in G and is_hgnc_node(G, g)}
    logger.info("Endothelial genes in-graph (HGNC): %d", len(endothelial))
    ...
    for raw_src, deg_csv in zip(args.sources, args.deg_csvs):
        ...
        if not src or src not in G or not is_hgnc_node(G, src):
            logger.info("SKIP %s->%s: source not in graph as HGNC node", raw_src, src)
            continue
        ...
        logger.info("%s->%s: endothelial targets in DEG universe = %d", raw_src, src, len(targets))
    ...
    pd.DataFrame(rows_1hop).to_csv(args.output_1hop, index=False)
    pd.DataFrame(rows_2hop).to_csv(args.output_2hop, index=False)
    logger.info("Done. 1-hop rows: %d -> %s", len(rows_1hop), args.output_1hop)
    logger.info("Done. 2-hop rows: %d -> %s", len(rows_2hop), args.output_2hop)
