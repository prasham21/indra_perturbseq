...
# Load the GWAS CSV
df = pd.read_csv("/Users/prashammarfatia/Downloads/gwas_endothelial_paths_url_enriched (1).csv")

print("="*80)
print("SEGREGATING GWAS CSV INTO 3 PARTS")
print("="*80)
print(f"Original GWAS CSV shape: {df.shape}\n")
...
print(f"Gene set sizes:")
print(f"  8-gene set: {len(genes_8)} genes")
print(f"  17-gene set: {len(genes_17)} genes")
print(f"  420-gene set: {len(genes_420)} genes\n")
...
print("="*80)
print(f"Rows with 8-gene set: {mask_8genes.sum():,}")
print(f"Rows with 17-gene set: {mask_17genes.sum():,}")
print(f"Rows with BOTH 8-gene and 17-gene sets: {(mask_8genes & mask_17genes).sum():,}")
print(f"Rows with EITHER 8-gene or 17-gene sets: {(mask_8genes | mask_17genes).sum():,}")
print(f"Residual rows (neither): {(~(mask_8genes | mask_17genes)).sum():,}\n")

# CREATE 8-GENE CSV
print("="*80)
print("CREATING 8-GENE CSV")
print("="*80)
...
print(f"Rows in 8-gene CSV: {len(df_8genes):,}")
print(f"Unique genes from 8-gene set found: {len(genes_8_found)} out of {len(genes_8)}")
print(f"  Genes found: {sorted(genes_8_found)}")
missing_8 = genes_8 - genes_8_found
if missing_8:
    print(f"  Genes NOT found: {sorted(missing_8)}")

output_8genes = "/Users/prashammarfatia/Downloads/gwas_8gene_set_.csv"
df_8genes.to_csv(output_8genes, index=False)
print(f"\n✓ 8-GENE CSV SAVED: {output_8genes}")
print(f"  Shape: {df_8genes.shape}\n")

# CREATE 17-GENE CSV
print("="*80)
print("CREATING 17-GENE CSV")
print("="*80)
...
print(f"Rows in 17-gene CSV: {len(df_17genes):,}")
print(f"Unique genes from 17-gene set found: {len(genes_17_found)} out of {len(genes_17)}")
print(f"  Genes found: {sorted(genes_17_found)}")
missing_17 = genes_17 - genes_17_found
if missing_17:
    print(f"  Genes NOT found: {sorted(missing_17)}")

output_17genes = "/Users/prashammarfatia/Downloads/gwas_17gene_set_.csv"
df_17genes.to_csv(output_17genes, index=False)
print(f"\n✓ 17-GENE CSV SAVED: {output_17genes}")
print(f"  Shape: {df_17genes.shape}\n")

# CREATE RESIDUAL CSV
print("="*80)
print("CREATING RESIDUAL CSV")
print("="*80)
...
print(f"Rows in residual CSV: {len(df_residual):,}")

# Check if any residual rows have 420-gene set genes
residual_with_420 = (df_residual['Genes_in_path_420gene_set'] != '').sum()
print(f"Residual rows with 420-gene set genes: {residual_with_420:,}")

output_residual = "/Users/prashammarfatia/Downloads/gwas_residual_.csv"
df_residual.to_csv(output_residual, index=False)
print(f"\n✓ RESIDUAL CSV SAVED: {output_residual}")
print(f"  Shape: {df_residual.shape}\n")

# FINAL SUMMARY
print("="*80)
print("FINAL SUMMARY")
print("="*80)
print(f"Original GWAS CSV: {len(df):,} rows")
print()
print(f"✓ 8-gene CSV: {len(df_8genes):,} rows")
print(f"  File: {output_8genes}")
print(f"  Genes covered: {len(genes_8_found)}/{len(genes_8)}")
print()
print(f"✓ 17-gene CSV: {len(df_17genes):,} rows")
print(f"  File: {output_17genes}")
print(f"  Genes covered: {len(genes_17_found)}/{len(genes_17)}")
print()
print(f"✓ Residual CSV: {len(df_residual):,} rows")
print(f"  File: {output_residual}")
print()
print(f"Verification: {len(df_8genes) - (mask_8genes & mask_17genes).sum()} (8-only) + {len(df_17genes) - (mask_8genes & mask_17genes).sum()} (17-only) + {(mask_8genes & mask_17genes).sum()} (overlap) + {len(df_residual)} (residual) = {len(df_8genes) - (mask_8genes & mask_17genes).sum() + len(df_17genes) - (mask_8genes & mask_17genes).sum() + (mask_8genes & mask_17genes).sum() + len(df_residual)} rows")
print(f"Note: Overlap rows appear in BOTH 8-gene and 17-gene CSVs")
print("="*80)